...
// 👉 Ajoute/retire tes flux ici (finance, crypto, tech, auto, etc.)
const SOURCES = [
...
function guessType(urlOrSource = "") {
  ...
  if (
    u.includes("immo") ||
    u.includes("immobilier") ||
    u.includes("realestate")
  )
    return "immo";
  if (
    u.includes("crypto") ||
  ...
  if (
    u.includes("bourse") ||
    u.includes("finance") ||
    u.includes("boursorama") ||
    u.includes("investir") ||
    u.includes("reuters")
  )
    return "finance";
  if (u.includes("tech") || u.includes("crunch") || u.includes("verge"))
    return "tech";
  return "gen";
}
