...
/* ----------------- Routing Auto vs Deals -----------------
   - Tu peux éditer ce mapping pour forcer un channel selon le domaine.
   - Tout ce qui ne matche pas part sur "deals".
----------------------------------------------------------- */
const DOMAIN_CHANNEL_MAP = [
  // AUTO
  { test: /autoscout24|lacentrale|caradisiac|autoplus/i, channel: "auto" },

  // (exemples pour immo/crypto/halal si tu veux plus tard)
...
export async function runCycle() {
  ...
  // 3) Enrich + routing + seuil par channel
  ...
  for (const it of filtered) {
    try {
      await publishTelegram(it, it.__channel); // "auto" ou "deals"
      published++;
    } catch (err) {
  ...
  console.log(
    `[Affiliator] mode=${MODE} total=${all.length} scored=${scored.length} avg=${avg} ` +
    `kept=${filtered.length} thresholds(global:${MIN.global}, deals:${MIN.deals || "-"}, auto:${MIN.auto || "-"})`
  );
